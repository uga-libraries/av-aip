...
for item in os.listdir():
  ...
  shutil.copy2(f'{item}/metadata/{item}_mediainfo.xml', f'{aip_staging}/mediainfo-xmls')

  # Makes pbcore xml and saves to local server.  
  subprocess.run(f'mediainfo -f --Output=PBCore2 --Language=raw {item}/objects/{item} > {aip_staging}/mediainfo-xmls/pbcore2-xml/{item}_mediainfo-pbcore2.xml', shell=True)
